...
This is the flow of data and control that happens when you perform the three main actions of HermesSwap:
...
2. Call the periphery contract's removeLiquidity function.
...
(`IHermesPair` and `HermesERC20`) or because it calls contracts that implement them.

```solidity
contract HermesPair is HermesERC20 {
```

This contract inherits from `HermesERC20`, which provides the the ERC-20 functions for the liquidity tokens.

```solidity
    using SafeMathHermes  for uint;
```
...
The solution that Hermesswap found is to use 224 bit values, with 112 bits for the integer part, and 112 bits
...
One of the biggest gas expenses of contracts is storage, which persists from one call of the contract
...
Hermesswap needs to transfer arbitrary ERC-20 tokens, which means calling ERC-20 contracts that may attempt to abuse the Hermesswap market that calls them.
...
We know that between the time `kLast` was calculated and the present no liquidity was added or removed (because we run this
...
Use the `HermesERC20._mint` function to actually create the additional liquidity tokens and assign them to `feeTo`.
...
tokens as a reward. It should be called from [a periphery contract](#HermesRouter02)
...
```solidity
        uint balance0 = IERC20Hermes(token0).balanceOf(address(this));
        uint balance1 = IERC20Hermes(token1).balanceOf(address(this));
...
If this is the first deposit, we check if the sender is the Migrator. Migrator is a service thats provide liquidity, using the desiredLiquidity function.

If the sender is not the Migrator service, we create `MINIMUM_LIQUIDITY` tokens and send them to address zero to lock them. They can
...
Use the `IERC20Hermes._mint` function to actually create the additional liquidity tokens and give them to the correct account.
...
Is should also be called [from a periphery account](#HermesRouter02).

```solidity
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        address _token0 = token0;                                // gas savings
        address _token1 = token1;                                // gas savings
        uint balance0 = IERC20Hermes(_token0).balanceOf(address(this));
        uint balance1 = IERC20Hermes(_token1).balanceOf(address(this));
...
This function is also supposed to be called from [a periphery contract](#HermesRouter02).
...
that anybody can call this function. You do not need permission from Hermesswap to create a new pair
exchange.
...
(this can be useful for layer 2 transactions.
...
[This contract](https://github.com/Hermes-defi/hermes-swap/blob/main/contracts/hermesswap/HermesERC20.sol) implements the
ERC-20 liquidity token.
...
The signature algorithm expects to get 256 bits to sign, so we use the `keccak256` hash function.
...
The router needs to know what factory to use, and for transactions that require WONE what WONE contract to use. These values are
...
```solidity
        (uint reserveA, uint reserveB) = HermesLibrary.getReserves(factory, tokenA, tokenB);
```
...
```solidity
            ...
            uint amountBOptimal = HermesLibrary.quote(amountADesired, reserveA, reserveB);
```

If we need to see what amounts will be, we get the optimal amount using
[this function](https://github.com/Hermesswap/Hermesswap-v2-periphery/blob/master/contracts/libraries/HermesLibrary.sol#L35).
...
```solidity
                ...
                uint amountAOptimal = HermesLibrary.quote(amountBDesired, reserveB, reserveA);
...
When a liquidity provider wants to provide liquidity to a Token/ONE pair exchange, there are a few differences. The
contract handles wrapping the ONE for the liquidity provider. There is no need to specify how many ONE the user wants
...
To deposit the ONE the contract first wraps it into WONE and then transfers the WONE into the pair. Notice that
...
```solidity
        ...
        (uint amount0, uint amount1) = IHermesPair(pair).burn(to);
```
...
Remove liquidity for ONE is almost the same, except that we receive the WONE tokens and then redeem them
for ONE to give back to the liquidity provider.
...
```solidity
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, HermesLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
...
These four variants all involve trading between ONE and tokens. The only difference is that we either receive ONE
from the trader and use it to mint WONE, or we receive WONE from the last exchange in the path and burn it, sending
the trader back the resulting ONE.
...
```solidity
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, HermesLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        uint balanceBefore = IERC20Hermes(path[path.length - 1]).balanceOf(to);
        ...
        ensure(deadline)
    {
        ...
        uint amountOut = IERC20Hermes(WONE).balanceOf(address(this));
...
```solidity
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IHermesPair(pairFor(factory, tokenA, tokenB)).getReserves();
...
[This library](https://github.com/Hermes-defi/hermes-swap/blob/main/contracts/hermesswap/libraries/TransferHelper.sol) adds success checks
...
```solidity
    ...
    function safeTransferONE(address to, uint256 value) internal {
        ...
        require(success, 'TransferHelper::safeTransferONE: ETH transfer failed');
    }
}
```
